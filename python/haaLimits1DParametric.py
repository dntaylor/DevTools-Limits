...
systproc = tuple([proc for proc in signalSplines + backgrounds if 'datadriven' not in proc])
allproc = tuple([proc for proc in signalSplines + backgrounds])
bgproc = tuple([proc for proc in backgrounds])
splineproc = tuple([proc for proc in signalSplines])
...
datacard = '{0}/mmmt_1d_parametric'.format(directory)
processes = {}
for h in hmasses:
    processes[signame.format(h=h,a='X')] = [splinename.format(h=h)] + backgrounds
limits.printCard(datacard,processes=processes,blind=False,saveWorkspace=True)
